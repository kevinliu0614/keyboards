...
const fingerCPM = [248, 312, 300, 324, 336,
                   336, 324, 300, 312, 248] # how many clicks can you do in a minute
...
const effortWeighting = (0.8, 1, 0, 0.5, 0) # dist, finger, row. 
# Also had room for other weightings but removed for simplicity
...
function doKeypress(myFingerList, myGenome, keyPress, oldFinger, oldHand, currentLayoutMap)
    # setup ~ get the key being pressed ~
...
function objectiveFunction(file, myGenome, currentLayoutMap)
    ...
    # ~ create hand ~
    myFingerList = zeros(10, 6) # (homeX, homeY, currentX, currentY, distanceCounter, objectiveCounter)

    for i in 1:length(currentLayoutMap)
    ...
    for currentCharacter in file
        ...
        # do keypress
        if keyPress !== nothing
            myFingerList, oldFinger, oldHand = doKeypress(
                myFingerList, myGenome, keyPress, oldFinger, oldHand, currentLayoutMap)
        end
    end
...
function runSA(
    ...
    while iteration <= num_iterations && temperature > 1.0
        ...
        # verbose && println(
        #     round(temperature, digits = 2), "\t", 
        #     round(bestObjective, digits=2), "\t", 
        #     round(newObjective, digits=2))
